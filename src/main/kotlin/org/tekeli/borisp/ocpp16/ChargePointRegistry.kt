...
interface ChargePointConnection : WebSocketSend {
...
class ChargePointRegistry {
    ...
    fun sendCall(chargePointId: String, action: String, payload: Map<String, Any>?): java.util.concurrent.CompletableFuture<OcppMessage> {
        ...
        val connection = sessionConnections[info.sessionId]
            ?: throw IllegalStateException("Connection not available for session: ${info.sessionId}")
        val dispatcher = OutboundCallDispatcher(connection, connection.responseAwaiter)
