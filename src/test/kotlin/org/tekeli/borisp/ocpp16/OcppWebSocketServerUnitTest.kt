...
class OcppWebSocketServerUnitTest {
    ...
    // === MUTATION KILLING TESTS for OcppWebSocketServer ===
    ...
    fun `persistenceService createSecurityLog called for SecurityEventNotification`() {
        ...
        try { s.onTextMessage("""[2,"p1","SecurityEventNotification",{"type":"Tampering","timestamp":"2024-01-01T00:00:00Z"}]""") } catch (_: RuntimeException) {}
        assertTrue(logCreated, "createSecurityLog must be called")
    }

    @Test
    fun `formation violation preserves exact error not default Payload validation failed`() {
        val s = OcppWebSocketServer().apply { chargePointId = "CP1" }
        val r = s.onTextMessage("""[2,"e1","BootNotification",{"chargePointVendor":"","chargePointModel":"M"}]""")
        assertTrue(r.contains("chargePointVendor is required"))
        assertFalse(r.contains("Payload validation failed"))
    }
